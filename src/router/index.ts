...
import SecIndex2 from '../views/Sec/index1.vue'
...
router.beforeEach((to: any, from: any, next: any) => {
  console.log(`----- ${appName} beforeEach()`, `【${from.path}】 => 【${to.path}】=> 【${to.fullPath}】=> ${w.qiankunMicroName}`);
  const tf = to.fullPath;
  // 设置允许子应用切换路由的条件，
  // 1、必须当前即将打开的页面是当前子系统
  // 2、当前存在该路由的，不然就报错
  if (w.qiankunMicroName == config.microName) {
    // 只取name=home的数据
